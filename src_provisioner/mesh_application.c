...
//#include "wiced_bt_mesh_vendor.h"
...
mesh_app_interrupt_handler_t  mesh_app_interrupt_handler  = NULL;
...
static uint8_t *p_240 = NULL;
static uint8_t *p_308 = NULL;
static uint8_t *p_396 = NULL;
static uint8_t *p_512 = NULL;
...
#include "wiced_rtos.h"
#include "src_light/raw_flash.h"
#include "src_light/log.h"
#include "src_light/storage.h"

#define SYNC_RESET_TIME   1000   //1s的定时
wiced_timer_t mesh_sync_reset_timer;
...
static void mesh_sync_reset_timer_callback(TIMER_PARAM_TYPE arg)
{
    extern void LightModelInitial(uint8_t onoff);
    extern int8_t uint16_to_percentage(uint16_t value);

    static uint16_t resetTimeCnt = 0;
    uint32_t current = 0;
    current = wiced_hidd_get_current_native_bt_clock();
    current = current*3125/10000;
    WICED_BT_TRACE("System Run time: %d ms\n",current);

    resetTimeCnt ++;
    if(resetTimeCnt == 1)
    {
        wiced_stop_timer(&mesh_sync_reset_timer);
        // led_controller_initial();
        LightModelInitial(0);
        LightFlash(120,3,50,0,0);
        wiced_start_timer(&mesh_sync_reset_timer, 4000);
    }
    else 
    {
        uint8_t temp;
        ...
        wiced_deinit_timer(&mesh_sync_reset_timer);
        ...
        wiced_result_t            result;
                ...
                // WICED_BT_TRACE("clear_flash_for_reset!  \n");
                if(WICED_TRUE == clear_flash_for_reset(&mesh_config,mesh_nvram_access))
                {
                    // WICED_BT_TRACE("reset write flash succeed!\n");
                }
                else
                {
                    // WICED_BT_TRACE("reset write flash fail!\n");
                }
                // temp = 255;
                // flash_app_write_mem(FLASH_ADDR_CNT, &temp, sizeof(temp));
                temp = configLIGHTRSTRESTARTCNT;
...
#endif
{
    ...
    //todo 识别工装按键，进入SMT检测
    value += wiced_hal_gpio_get_pin_input_status((uint32_t)*platform_button[WICED_PLATFORM_BUTTON_2].gpio);
    if (value == 0)
    {
    ...
    // else
    {
        ...
        #elif (CHECK_POWEROFF_VALUE) && (!CHECK_POWEROFF_INTT)
        #elif (!CHIP_EVK)
        //#error "Err: please check CHECK_POWEROFF_VALUE and CHECK_POWEROFF_INTT, must only one be set 1 !!!"
        #endif
#endif
    }
#ifndef MESH_HOMEKIT_COMBO_APP
...
// #if MESH_START_SYNC != 0
//     uint8_t temp;
//     if(0 == flash_app_read_mem(FLASH_ADDR_CNT, &temp, sizeof(temp)))
//     {
//         temp = 255;
//     }
//     if(temp)
//         temp--;
//     WICED_BT_TRACE("temp = %d\n",temp);
//     if(!(temp > configLIGHTRSTCNT))
//     {
//         WICED_BT_TRACE("start sync timer\n");
//     }
//     else
// #endif
    {
    ...
    //     current = current*3125/10000;
    //     WICED_BT_TRACE("System Run time: %d ms\n",current);
    //     //TODO  注意正式产品需要注意越界的问题
    //     // if(SYNC_RESET_TIME > current)
    //     // {
    //     //     current = SYNC_RESET_TIME - current;
    //     // }
    //     // else
    //     // {
    //     //     current = 50;
    //     // }
    //     current = SYNC_RESET_TIME - current;
    //     //时间不足时，启动50ms定时，否则按剩余时间启动同步定时器，使得灯同步闪烁
    //     if(current < 0)
    //     {
    //         current = 50;
    //     }
    //     wiced_init_timer(&mesh_sync_reset_timer,mesh_sync_reset_timer_callback, 0, WICED_MILLI_SECONDS_PERIODIC_TIMER);
    //     wiced_start_timer(&mesh_sync_reset_timer, current);
    //     WICED_BT_TRACE("System timer start: %d ms\n",current);
    // }
#endif
...
wiced_result_t mesh_management_cback(wiced_bt_management_evt_t event, wiced_bt_management_evt_data_t *p_event_data)
{
    ...
    case BTM_ENABLED_EVT:
...
#if MESH_START_SYNC != 0
        {
            ...
            if((!(temp > configLIGHTRSTCNT)) && (temp > configLIGHTRSTRESTARTCNT))
            {
                int32_t current = 0;
                current = wiced_hidd_get_current_native_bt_clock();
                current = current*3125/10000;
            ...
            {
                extern void LightSetOnPoweron(uint8_t);
                uint8_t lighton = 0;

                //恢复出厂设置时，灯上电状态临时切换为开
                if(temp <= configLIGHTRSTRESTARTCNT)
                {
                    temp = 255;
                    flashWriteBlockSave(FLASH_ADDR_CNT, &temp, sizeof(temp));
                    lighton = 1;
...
//extern uint8_t node_uuid_magic_bytes[4];
void mesh_application_gen_uuid(uint8_t *uuid)
{
    ...
    extern void wiced_bt_mesh_vendor_gen_device_uuid(uint8_t *uuid);
    wiced_bt_mesh_vendor_gen_device_uuid(uuid);
#else
    for (i = 0; i < 2000; i++)
    {
...
{
    wiced_result_t result;
    WICED_BT_TRACE("mesh_factory_reset...\n");
    ...
    // mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_PROVISIONER, NULL, 0, &result);
#ifdef USE_VS_NVRAM
...
void mesh_ota_firmware_upgrade_status_callback(uint8_t status)
{
    ...
    else if (status == OTA_FW_UPGRADE_STATUS_FINISH)
    {
        #if (LIGHT == 1) || (LIGHT == 2)
            extern void storage_save_upgradeflag(void);
            wiced_hal_wdog_restart();
            storage_save_upgradeflag();
        #endif
    }
}
...
//Test timer start
   // wiced_init_timer(&test_timer,&testTimerCb,0,WICED_MILLI_SECONDS_PERIODIC_TIMER);
   // //wiced_start_timer(&test_timer,50);
//    wiced_hal_gpio_configure_pin(*platform_led[WICED_PLATFORM_LED_2].gpio, GPIO_OUTPUT_ENABLE, GPIO_PIN_OUTPUT_HIGH);
   // flag_test = 0;
//Test timer end
    // p_240 = wiced_memory_permanent_allocate(240);
    // p_308 = wiced_memory_permanent_allocate(308);
    // p_396 = wiced_memory_permanent_allocate(396);
    // p_512 = wiced_memory_permanent_allocate(512);

    // if(p_240 != NULL){
    //     memset(p_240,0x55,240);
    // }else{
    //     LOG_DEBUG("P_240 err\n");
    // }
    // if(p_308 != NULL){
    //     memset(p_308,0x55,308);
    // }else{
    //     LOG_DEBUG("P_308 err\n");
    // }
    // if(p_396 != NULL){
    //     memset(p_396,0x55,396);
    // }else{
    //     LOG_DEBUG("P_396 err\n");
    // }
    // if(p_512 != NULL){
    //     memset(p_512,0x55,512);
    // }else{
    //     LOG_DEBUG("P_512 err\n");
    // }
...
//#endif
    ...
    */
...
#endif  
        LOG_INFO("***** Free mem after app_init:%d\n", wiced_memory_get_free_bytes());
        return;
    }
    ...
    // // WICED_BT_TRACE("provision_status:%d result:%d len:%d\n", provision_status, result, len);
    // if (result == WICED_BT_SUCCESS && len == 1 && provision_status == 1)
    // {
    ...
    //     mesh_application_factory_reset();
    // }
    if (result == WICED_BT_SUCCESS && len == 1)
    {
        // Looks like the board reboot or stucked in the middle of provisioning/configuring.
        // Do a factory reset to clean up everything.
        if(provision_status == MESH_APP_PROVISION_STATUS_PROVISIONING)
        {
        ...
        else if(provision_status == MESH_APP_PROVISION_STATUS_PRORESTART)
        {
            mesh_nvram_access(WICED_TRUE, NVRAM_ID_LOCAL_PROVISION, NULL, 0, &result);
            //LightFlash(60, 0x7FFF, 50, 100);
        }
    }
...
static void mesh_adv_report(wiced_bt_ble_scan_results_t *p_scan_result, uint8_t *p_adv_data)
{
    ...
    else if (p_scan_result->ble_evt_type == BTM_BLE_EVT_NON_CONNECTABLE_ADVERTISEMENT)
    {
            ...
            && (p_adv_data[2] == 0x04) && (p_adv_data[3] == 0x08) )
        {
            ...
            // LOG_DEBUG("[%d.%d]Andon_adv rec\n",sec,ms);
            (*adv_manufacturer_handler)(p_scan_result,p_adv_data);
            // if((*adv_manufacturer_handler)(p_scan_result,p_adv_data))
            // {
            //     tick = wiced_bt_mesh_core_get_tick_count();
            //     sec  = (tick / 1000) % 1000;
            //     ms  = tick % 1000;
            //     LOG_DEBUG("Andon adv done\n");
            //     return;
            // }
            tick = wiced_bt_mesh_core_get_tick_count();
            sec  = (tick / 1000) % 1000;
            ms  = tick % 1000;
            // LOG_DEBUG("[%d.%d]Andon_adv done\n",sec,ms);
            return;
        }
...
void mesh_interrupt_handler(void* user_data, uint8_t pin)
{
    ...
    if (mesh_app_interrupt_handler != NULL)
    {
        (*mesh_app_interrupt_handler)(user_data, pin, value);
    }
    /*
    else
#if !defined(CYW43012C0) && !defined(CYW20706A2) && !defined(CYW20719B0)
...
wiced_bool_t mesh_publication_callback(uint8_t elem_idx, uint16_t company_id, uint16_t model_id, uint16_t period)
{
    ...
    WICED_BT_TRACE("pub_cb: idx:%d cid:%x model:%x period:%X\n", elem_idx, company_id, model_id, period);
...
void mesh_state_changed_cb(wiced_bt_mesh_core_state_type_t type, wiced_bt_mesh_core_state_t *p_state)
{
    ...
    case WICED_BT_MESH_CORE_STATE_NODE_STATE:
        ...
        {
            extern wiced_bool_t is_provisioner;
            WICED_BT_TRACE("2......\n");
            wiced_bt_mesh_app_func_table.p_mesh_app_init(node_authenticated);
            // if(is_provisioner == WICED_FALSE)
            // {
            //     adv_pair_disable();
            // }
        }
    ...
    case WICED_BT_MESH_CORE_STATE_LPN_SCAN:
        ...
        WICED_BT_TRACE("%sLPN_SCAN scan:%d\n", str, p_state->lpn_scan);
#ifndef PTS
    ...
    case WICED_BT_MESH_CORE_STATE_LPN_SLEEP:
        WICED_BT_TRACE("%sLPN_SLEEP timeout:%d\n", str, p_state->lpn_sleep);
    ...
    case WICED_BT_MESH_CORE_STATE_FRND_FRIENDSHIP:
        WICED_BT_TRACE("%sFRND_FRIENDSHIP established:%d addr:%x\n", str, p_state->frnd.established, p_state->frnd.addr);
    ...
    case WICED_BT_MESH_CORE_STATE_APP_KEY_ADD:
        WICED_BT_TRACE("%sAPP_KEY_ADD net_key_idx:%x app_key_idx:%x\n", str, p_state->app_key.net_key_index, p_state->app_key.index);
#ifdef ENABLE_SELF_CONFIG_MODEL
...
void (*mesh_provisioner_timer_cb)(void) = NULL;
void (*mesh_reset_timer_cb)(void) = NULL;
void (*store_cache_timer_cb)(void) = NULL;
...
void mesh_app_timer(uint32_t arg)
{
    ...
    //     if(*(p_240+i) != 0x55)
    //     {
    //         LOG_DEBUG("memhole 240 err!!!!!!");
    //         while(1){
    //             wiced_hal_wdog_restart();
    //             wiced_rtos_delay_milliseconds(6000, KEEP_THREAD_ACTIVE);
    //             LOG_DEBUG("memhole 240 err!!!!!!");
    //         }
    //     }
    // }
    ...
    {
        extern void memory_hole_info(void);
        deb_print_buf_use();
        memory_hole_info();
    }
#endif
    ...
    if (mesh_provisioner_timer_cb != NULL)
    {
        (*mesh_provisioner_timer_cb)();
    }
    ...
    if (store_cache_timer_cb != NULL)
    {
        (*store_cache_timer_cb)();
    }
}
...
uint8_t wiced_bt_mesh_app_get_provisioner(void)
{
...
void wiced_bt_mesh_app_set_configured(void)
{
    ...
    if(is_provisioner == WICED_TRUE)
    {
        LOG_DEBUG("myself is pro_er\n");
        node_authenticated = WICED_TRUE;
    }
    node_authenticated = WICED_TRUE;
    mesh_app_gatt_db_init(node_authenticated);
